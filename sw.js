...
function idbGetLog() {
  return idbGetValue('prayerLog').then((v) => v ? JSON.parse(v) : {});
}

async function countMissedDays(log) {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const startStr = await idbGetValue('startDate');
  let start;
  if (startStr) {
...
async function notifyIfMissed() {
  ...
  const missed = await countMissedDays(log);
  if (missed > 0) {
